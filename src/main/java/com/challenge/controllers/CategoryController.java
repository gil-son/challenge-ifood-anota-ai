...
public class CategoryController {
    ...
    public ResponseEntity<Category> insert(@RequestBody CategoryDTO categoryData){
        return ResponseEntity.status(HttpStatusCode.valueOf(201)).body(this.service.insert(categoryData));
    }
    ...
    public ResponseEntity<Category> update(@PathVariable("id") String id, @RequestBody CategoryDTO categoryData){
        return ResponseEntity.ok().body(this.service.update(id, categoryData));
    }
